...
export class ProviderComponent implements OnInit, OnDestroy {
  ...
  constructor(private authService:AuthService, private pgs:PlayerGetService, private cgs:CompGetService, private lps:LineupPostService) {
    ...
    this.userName = "ABC"; // this.userData.first_name + " " + this.userData.last_name;
  ...
  updateLineup = () => {
    var lineup = this.lineupPlayers.map(player => (player['Player'] == null) ? '' : player['Player']['OperatorPlayerName']);
    this.lps.updateLineup(this.currentOperator, this.currentSport, this.currentSlate, this.userName, this.lineupID, lineup).subscribe();
  }
  deleteLineup = (id) => {
  ...
  loadLineup = (id) => {
    ...
    var lineupPlayers2 = currLineup.map(playerName => this.players[this.memIndex(allPlayerNames, playerName)]);
