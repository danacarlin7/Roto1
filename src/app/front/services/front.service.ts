...
export class FrontService {
  ...
  getHeaders(): Headers {
    ...
    headers.append("content-tsype", "application/json");
    if (this.getToken()) {
  ...
  retrieveNews(sportType: string, timePeriod: string = "30days"): Observable<any> {
    return this.http.get(environment.api_end_point + "fetchNews?sport=" + sportType + "&since=" + timePeriod,
  ...
  retrieveDailyLineups(sportType: string, timePeriod: string = "cyear"): Observable<any> {
    return this.http.get(environment.api_end_point + "fetchLineup?sport=" + sportType + "&since=" + timePeriod,
      { headers: this.getHeaders() })
      .map((reponse: Response) => reponse.json())
      .catch(error => {
        this.handelError(error.json());
  ...
  subscribePlan(token, plan_id, coupon = ""): Observable<any> {
    console.log(coupon);
    return this.http.post(environment.api_end_point + "api/subscribe",
      { token, plan_id, coupon }, { headers: this.getHeaders() })
      .map((response: Response) => response.json());
  }

  unsubscribePlan(subscribe_id, at_period_end) {
    return this.http.put(environment.api_end_point + "api/unsubscribe/" + subscribe_id,
      { at_period_end: at_period_end }, { headers: this.getHeaders() })
      .map((response: Response) => response.json());
  }
